export const cdn_dynamic_content = {
  "id": "cdn-dynamic-content",
  ...
  "deepDive": [
    ...
    {
      "heading": "Dynamic Content Acceleration Techniques",
      "text": "TLS connection reuse: CDN maintains persistent connections to origin, reducing connection overhead. TCP optimization: optimized congestion control algorithms reduce latency. Route optimization: CDN uses real-time internet health data to find the best path to origin. Preconnect: CDN establishes early connections to speed up dynamic requests."
    },
    {
      "heading": "Edge Computing for Dynamic Content",
      "text": "Cloudflare Workers, Fastly Compute@Edge, and CloudFront Functions can generate dynamic responses at the edge without hitting the origin. Use cases: A/B testing (serve variant based on cookie), personalization (modify HTML based on geolocation), API aggregation (combine multiple API responses), authentication (validate JWT at edge)."
    },
    {
      "heading": "Caching Dynamic Content (Selectively)",
      "text": "Cache personalized content with Vary header (Vary: Cookie, Authorization). Use short TTLs (30-60 seconds) for semi-dynamic content. Cache fragments via Edge Side Includes (ESI). Use surrogate keys to invalidate groups. Cache API responses with user-specific cache keys. Only cache if the benefit outweighs the complexity."
    },
    {
      "heading": "Dynamic vs Static Content Separation",
      "text": "Best practice: separate dynamic and static content on different URL patterns. Static: /static/, /assets/, /images/. Dynamic: /api/, /app/, /dashboard/. This allows different CDN configurations (TTL, cache policies, origin groups) for each. Many CDNs support path-based behavior configuration."
    }
  ],
  ...
  "mcqQuestions": [
    {
      ...
      "options": [
        ...
        "Content that varies by user/session/context",
        "Only video content",
        "Only HTML content"
      ],
      "answer": 1,
      "explanation": "Dynamic content varies by user, session, time, or context."
    },
    {
      "question": "Why is caching dynamic content challenging?",
      "options": [
        "It is too large",
        "It varies by user/context creating many cache entries",
        "It is encrypted",
        "It expires too fast"
      ],
      "answer": 1,
      "explanation": "Each variation of dynamic content needs a separate cache entry, making caching inefficient."
    },
    {
      "question": "What does the Vary header do?",
      "options": [
        "Changes the content encoding",
        "Tells CDN to cache different versions based on request headers",
        "Varies the TTL dynamically",
        "Changes the origin server"
      ],
      "answer": 1,
      "explanation": "Vary tells CDNs to cache different versions based on request headers like Cookie or Authorization."
    },
    {
      "question": "What is an Edge Worker used for?",
      "options": [
        "Cleaning edge cache",
        "Running code at edge for dynamic responses",
        "Managing DNS records",
        "Compressing images"
      ],
      "answer": 1,
      "explanation": "Edge Workers run custom code at CDN edge locations for dynamic content generation."
    },
    {
      "question": "What is ESI?",
      "options": [
        "Edge Side Includes — dynamic content assembly from cached fragments",
        "Extra Server Infrastructure",
        "Elastic Server Integration",
        "Edge Security Interface"
      ],
      "answer": 0,
      "explanation": "ESI allows composing dynamic content from cached fragments at the edge."
    },
    {
      "question": "What CDN feature helps with selective dynamic content caching?",
      "options": [
        "Cache-Control: private",
        "s-maxage with Vary header",
        "Increasing TTL",
        "Disabling cache entirely"
      ],
      "answer": 1,
      "explanation": "Using s-maxage with Vary header allows selective caching of dynamic content with user-specific variations."
    }
  ]
};
