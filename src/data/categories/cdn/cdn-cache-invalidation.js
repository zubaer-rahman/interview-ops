export const cdn_cache_invalidation = {
  ...
  "laymanDefinition": "Cache invalidation is like recalling a batch of product packaging when you realize the ingredient list has an error. You have to go to every store (edge server) and remove all the old boxes. The tricky part: you need to know which boxes are in which stores, and you must replace them before customers see outdated information.",
  "deepDive": [
    {
      "heading": "Invalidation Methods",
      "text": "Exact path: /images/banner.jpg — invalidates one file. Wildcard: /images/* — invalidates all images. Directory: /css/ — invalidates all CSS files. Tag-based: invalidate by content tags like \"v2.0-release\". API-based: through CDN provider APIs. Some providers charge per invalidation path or have monthly limits."
    },
    {
      "heading": "Versioned Filename Strategy (Cache Busting)",
      "text": "Instead of invalidating old cache, serve different filenames for updated content: style.abc123.css instead of style.css. The old content remains cached (no users request it). The new filename automatically gets its own cache entries. This is the most efficient approach — zero invalidation cost, zero cache miss penalty for updated content."
    },
    {
      "heading": "CDN Invalidation Policies",
      "text": "CloudFront: free first 1000 paths/month, $0.005/path after. Fastly: instant purge, no charge (with limits). Cloudflare: unlimited purges via API, UI, or tag. Akamai: instant invalidation or CP code-based. Varnish: PURGE HTTP method (if configured). Most CDNs propagate invalidation within seconds globally."
    },
    {
      "heading": "Purge Propagation Time",
      "text": "Invalidation must propagate to all edge servers worldwide. CloudFront: typically <60 seconds. Fastly: <150ms for instant purge. Cloudflare: <30 seconds. Akamai: <10 seconds. During propagation, some users may still receive the old content (eventual consistency). This is why versioned filenames are preferred."
    },
    {
      "heading": "Tag-Based Invalidation",
  ...
  "mcqQuestions": [
    {
      ...
      "answer": 1,
      "explanation": "Versioned filenames avoid invalidation entirely — old content is never requested again."
    },
    {
      "question": "What is the CloudFront invalidation cost?",
      "options": [
        "Free unlimited",
        "First 1000 free, then $0.005/path",
        "$0.01 per path",
        "Free up to 10000 paths"
      ],
      "answer": 1,
      "explanation": "CloudFront offers 1000 free invalidations per month, then charges $0.005 per path."
    },
    {
      "question": "Which CDN offers near-instant purge (<150ms)?",
      "options": [
        "CloudFront",
        "Fastly",
        "Cloudflare",
      ...
      "answer": 1,
      "explanation": "Fastly offers near-instant purge, typically completing in under 150 milliseconds."
    },
    {
      "question": "What are Surrogate-Keys used for?",
      "options": [
        "Content compression",
      ...
      "answer": 1,
      "explanation": "Surrogate-Keys enable tag-based invalidation in Fastly."
    },
    {
      "question": "What is the saying about cache invalidation?",
      "options": [
        "\"Cache is king\"",
      ...
      "answer": 1,
      "explanation": "Phil Karlton famously said this about the difficulty of cache invalidation."
    },
    {
      "question": "What does a content hash-based filename prevent?",
      "options": [
        "Compression errors",
      ...
      "answer": 1,
      "explanation": "Content hash filenames change when content changes, so old cached versions are never requested again."
    }
  ]
};
