export const log_structured = {
  ...
  "laymanDefinition": "Structured logging is like filling out a standardized form instead of writing a paragraph. A form has named fields: \"Date:\", \"Customer:\", \"Issue:\". You can instantly search all forms for \"Customer: Alice\". Unstructured logging is like a diary entry: \"Today Alice came in with a problem, it happened around 3 PM.\" Harder to search and analyze.",
  "deepDive": [
    {
      "heading": "JSON Log Format",
      "text": "{\"@timestamp\": \"2024-06-15T10:30:00Z\", \"level\": \"error\", \"logger\": \"UserService\", \"message\": \"Failed to create user\", \"userId\": 42, \"error\": {\"code\": \"DUPLICATE_EMAIL\", \"stack\": \"...\"}}. Benefits: parsers can index each field, dashboards can aggregate by level, alerts can trigger on specific conditions."
    },
    {
      "heading": "Log Levels in Structured Logging",
      "text": "trace: detailed debugging (development only). debug: diagnostic info for troubleshooting. info: normal application events (request started, user created). warn: unexpected but handled issues (rate limit exceeded). error: failures requiring attention (DB connection lost). fatal: unrecoverable errors (process exit)."
    },
    {
      "heading": "Contextual Logging (Child Loggers)",
      "text": "Create child loggers pre-populated with context: const childLogger = logger.child({ service: \\'orders\\', requestId: req.id }). Every log from this child automatically includes service and requestId fields. Eliminates repetitive manual field injection. Patterns: per-request child logger in middleware, per-service child logger at startup."
    },
    {
      "heading": "Structured Logging in Microservices",
  ...
  "codeExamples": [
    {
      ...
      "description": "Pino outputs structured JSON with automatic timestamp, level, and custom context fields."
    },
    {
      "title": "Winston Structured Logger (Node.js)",
      "useCase": "Popular alternative with transports.",
      "code": "const winston = require('winston');\n\nconst logger = winston.createLogger({\n  level: 'info',\n  format: winston.format.combine(\n    winston.format.timestamp(),\n    winston.format.errors({ stack: true }),\n    winston.format.json()\n  ),\n  defaultMeta: { service: 'order-service' },\n  transports: [\n    new winston.transports.Console(),\n    new winston.transports.File({\n      filename: 'logs/error.log', level: 'error'\n    })\n  ]\n});\n\nlogger.error('Database connection failed', {\n  dbHost: process.env.DB_HOST,\n  retryAttempt: 3\n});",
      "description": "Winston provides flexible transports and formatting with structured JSON output."
    },
    {
      "title": "Child Logger with Request Context",
      "useCase": "Automatic context via middleware.",
      "code": "// Middleware: create child logger per request\napp.use((req, res, next) => {\n  req.log = logger.child({\n    requestId: uuidv4(),\n    method: req.method,\n    url: req.url,\n    ip: req.ip\n  });\n  next();\n});\n\n// Route handler — no repetitive context injection\napp.get('/api/users/:id', async (req, res) => {\n  req.log.info({ userId: req.params.id }, 'Fetching user');\n  try {\n    const user = await db.findUser(req.params.id);\n    req.log.info('User found');\n    res.json(user);\n  } catch (err) {\n    req.log.error({ err }, 'Failed to fetch user');\n    res.status(500).json({ error: 'Internal error' });\n  }\n});",
      "description": "Child loggers automatically include request context in every log entry without manual injection."
    },
    {
      "title": "Structured Error Logging with Stack Traces",
      "useCase": "Log full error details.",
      ...
      "description": "Error serializers capture full stack traces and error details in structured format."
    },
    {
      "title": "AsyncLocalStorage for Automatic Context",
      "useCase": "No need to pass logger through every function.",
      ...
      "description": "AsyncLocalStorage propagates the logger context automatically without passing it through parameters."
    }
  ],
  "mcqQuestions": [
    {
      "question": "What format does structured logging use?",
      "options": [
        "CSV",
        "JSON",
        "XML",
      ...
      "answer": 1,
      "explanation": "Structured logging most commonly uses JSON format with named fields."
    },
    {
      "question": "Which Node.js logger is fastest?",
      "options": [
        "Winston",
        "Bunyan",
        "Pino",
      ...
      "answer": 2,
      "explanation": "Pino is significantly faster due to minimal serialization overhead."
    },
    {
      "question": "What is a child logger?",
      "options": [
        "A logger for children",
        "A logger with pre-populated context fields",
        "A separate log file",
        "A logger that filters by level"
      ],
      "answer": 1,
      "explanation": "Child loggers inherit parent configuration and add predefined context fields automatically."
    },
    {
      "question": "What should be included in every log?",
      "options": [
      ...
      "answer": 0,
      "explanation": "Every structured log should include timestamp, level, and a descriptive message plus relevant context."
    },
    {
      "question": "Why log to stdout in containers?",
      "options": [
        "Better performance",
        "Docker captures stdout; shippers collect from Docker",
        "It is the default",
        "Files are not allowed"
      ],
      "answer": 1,
      "explanation": "Docker and Kubernetes capture stdout/stderr; using stdout is the container-native logging pattern."
    },
    {
      "question": "What log level indicates failure?",
      "options": [
        "info",
        "warn",
        "error",
      ...
      "answer": 2,
      "explanation": "Error level indicates a failure that needs investigation. Fatal indicates unrecoverable errors."
    },
    {
      "question": "Structured Logging — What reduces errors most?",
      "options": [
        "Automation",
        "Manual processes",
        "Rushing",
    ...
    {
      "question": "Structured Logging — What improves speed?",
      "options": [
    ...
    {
      "question": "Structured Logging — What is key for monitoring?",
      "options": [
    ...
    {
      "question": "Structured Logging — What ensures quality?",
      "options": [
