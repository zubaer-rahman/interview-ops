export const git_revert = {
  ...
  "laymanDefinition": "Git revert is like a time machine that adds a new event to undo a past event rather than erasing the past. If you baked a cake (commit) and it turns out terrible revert adds a remove cake from kitchen event to the timeline. The original bake cake event is still in the history you can see it. Reset would be erasing the memory of baking entirely.",
  "deepDive": [
    {
      "heading": "How Revert Works",
      "text": "Revert computes the inverse diff of the target commit and applies it as a new commit. If commit A added 10 lines revert A creates a commit that removes those 10 lines. If commit A removed a file revert A restores it (if no conflicting changes). The original commit remains in history."
    },
    {
      "heading": "Reverting a Single Commit",
      "text": "git revert <sha>. Opens editor for commit message (default: \"Revert <commit-title>\"). Works on any commit not just HEAD. No-edit: git revert --no-edit <sha>. No-commit: git revert -n <sha> (stage changes but don commit allows combining multiple reverts)."
    },
    {
      "heading": "Reverting a Merge Commit",
      "text": "Merge commits have two parents. git revert -m 1 <merge-sha> reverts to the state of parent 1 (the branch you were on). -m 2 reverts to parent 2 (the branch you merged). Without -m git refuses to revert a merge (ambiguous)."
    },
    {
      "heading": "Revert Conflicts",
      "text": "If later commits modified the same code revert may conflict. Resolution is like a normal merge conflict: edit git add git revert --continue. Or abort: git revert --abort. To skip a conflicted revert: git revert --skip."
    },
    {
      "heading": "Revert vs Reset vs Restore",
      "text": "Revert: safe for public history creates new commit. Reset: moves branch pointer rewrites history (use only on local/unpushed branches). Restore: restores working tree/index files (new Git 2.23+). For public/shared branches always use revert."
    }
  ],
  ...
  "mcqQuestions": [
    {
      ...
      "answer": 1,
      "explanation": "Revert creates a new inverse commit safe for shared history."
    },
    {
      "question": "How is revert different from reset?",
      "options": [
        "Revert rewrites history",
        "Revert adds a new commit reset moves pointer",
        "They are the same",
        "Reset is safer for shared branches"
      ],
      "answer": 1,
      "explanation": "Revert adds history (safe) reset rewrites history (unsafe for shared branches)."
    },
    {
      "question": "What flag reverts a merge commit?",
      "options": [
        "-m (parent number)",
        "-p (parent)",
        "--merge",
      ...
      "answer": 0,
      "explanation": "-m specifies which parent to revert to: -m 1 or -m 2."
    },
    {
      "question": "What happens if a revert causes conflicts?",
      "options": [
        "Revert is automatically aborted",
        "Resolve conflicts and git revert --continue",
        "The commit is skipped",
        "Git force-reverts anyway"
      ],
      "answer": 1,
      "explanation": "Handle revert conflicts like merge conflicts: resolve add continue."
    },
    {
      "question": "How do you revert without opening an editor?",
      "options": [
      ...
      "answer": 1,
      "explanation": "--no-edit uses the default revert message without opening an editor."
    },
    {
      "question": "Why use revert instead of reset on main?",
      "options": [
        "Faster operation",
        "History is preserved for collaboration",
        "Smaller repository size",
        "No conflicts possible"
      ],
      "answer": 1,
      "explanation": "Revert creates a new commit (preserves history) which is safe for shared branches."
    },
    {
      "question": "Git Revert — What reduces errors most?",
      "options": [
        "Automation",
        "Manual processes",
        "Rushing",
    ...
    {
      "question": "Git Revert — What improves speed?",
      "options": [
    ...
    {
      "question": "Git Revert — What is key for monitoring?",
      "options": [
    ...
    {
      "question": "Git Revert — What ensures quality?",
      "options": [
