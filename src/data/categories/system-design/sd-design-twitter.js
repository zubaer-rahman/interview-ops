export const sd_design_twitter = {
  "id": "sd-design-twitter",
  ...
  "laymanDefinition": "Twitter feed is like a personalized newspaper. PUSH model = the newspaper is printed and delivered to your door every morning (pre-computed). You open it and read immediately. PULL model = you go to the newsstand and pick from available papers when you want to read. Twitter uses PUSH for regular users (pre-computed feed) and PULL for celebrities (million followers would crush PUSH).",
  "deepDive": [
    {
      "heading": "Fan-Out on Write (Push)",
      "text": "When user tweets: insert tweet into EACH follower\\'s feed timeline. Read: just fetch timeline from cache. Pro: fast reads (O(1)). Con: expensive writes for celebrities (millions of writes per tweet). Twitter: PUSH for users with < 10K followers. Hybrid: PUSH for regular users, PULL for celebrities."
    },
    {
      "heading": "Fan-Out on Read (Pull)",
      "text": "When user views feed: fetch recent tweets from all followed users, merge, sort, return. Pro: no write amplification. Con: slow reads (merge K lists). K = number of followed users. Good for: celebrities (few writes, many readers), infrequent readers. Hybrid approach: pre-compute for active followers on tweet."
    },
    {
      "heading": "Timeline Storage and Caching",
      "text": "Timeline: Redis sorted sets (feed:userId, score = tweet timestamp). Pre-computed feed stored for 24h. Tweets themselves in DB/cache. Cache: Redis cluster for hot feeds (active users first page). DB: Cassandra/PostgreSQL for tweet storage. Pagination: cursor-based (tweet ID/timestamp) not offset-based."
    },
    {
      "heading": "Trending Topics",
      "text": "Batch processing: MapReduce/Spark every 5 minutes. Count hashtags in recent tweets. Normalize by baseline popularity. Demote topics that have been trending too long. Geo-specific trends: separate per region. Real-time: Storm/Flink for streaming trend detection."
    }
  ],
  ...
  "codeExamples": [
    ...
    },
    {
      ...
      "description": "Timeline generation — pre-computed feed + celebrity pull."
    },
    {
      "title": "Tweet Storage (Cassandra)",
      "useCase": "High-write schema.",
  ...
  "mcqQuestions": [
    {
      ...
      "answer": 1,
      "explanation": "Reads are O(1) — pre-computed feed."
    },
    {
      "question": "PULL model better for?",
      "options": [
        "Regular users",
        "Celebrities",
        "All users",
      ...
      "answer": 1,
      "explanation": "Celebrities — avoids millions of writes per tweet."
    },
    {
      "question": "Twitter uses which hybrid approach?",
      "options": [
        "PUSH for all",
        "PUSH for regular, PULL for celebrities",
        "PULL for all",
        "Neither"
      ],
      "answer": 1,
      "explanation": "PUSH for regular users, PULL for celebrities."
    },
    {
      "question": "How to store timeline?",
      "options": [
        "MySQL",
        "Redis sorted set",
        "S3",
      ...
      "answer": 1,
      "explanation": "Redis sorted set with timestamp as score."
    },
    {
      "question": "Trending topics computed how?",
      "options": [
        "Real-time every request",
        "Batch (Spark every 5 min)",
        "On tweet creation",
        "Manual"
      ],
      "answer": 1,
      "explanation": "Batch processing every 5 minutes."
    },
    {
      "question": "Fan-out writes per celebrity tweet?",
      "options": [
        "Few",
        "Millions",
        "Zero",
      ...
      "answer": 1,
      "explanation": "Millions of writes if fan-out for celebrities — avoided via PULL."
    },
    {
      "question": "Design Twitter Feed — What reduces errors most?",
      "options": [
        "Automation",
        "Manual processes",
        "Rushing",
    ...
    {
      "question": "Design Twitter Feed — What improves speed?",
      "options": [
    ...
    {
      "question": "Design Twitter Feed — What is key for monitoring?",
      "options": [
    ...
    {
      "question": "Design Twitter Feed — What ensures quality?",
      "options": [
