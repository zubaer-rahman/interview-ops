export const sd_database_sharding = {
  ...
  "laymanDefinition": "Sharding is like splitting a library's books across multiple buildings by first letter. A-D in Building 1, E-K in Building 2, etc. To find a book, you know which building (shard) based on the letter (shard key). But finding all books by \"Smith\" (cross-shard query) requires visiting every building. Adding Building 5 means reorganizing many books (resharding).",
  "deepDive": [
    {
      "heading": "Choosing a Shard Key",
      "text": "Goal: even data distribution, match access patterns. User ID: good for user-centric apps — each user\\'s data on one shard. Geographic: region-based — good for locality. Time-based: by date — good for time-series but hot spot on current data. Avoid: monotonically increasing keys (hot shard). High-cardinality, evenly distributed keys are best."
    },
    {
      "heading": "Hash-Based Sharding",
      "text": "shard = hash(shard_key) % N. Even distribution. Problem: adding/removing shards changes N → most data needs rehashing (resharding). Solution: Consistent Hashing — minimizes remapping when N changes. Each shard handles a range of hash space. Virtual nodes for better distribution."
    },
    {
      "heading": "Range-Based Sharding",
      "text": "shard = key falls in range. User IDs 1-10000 → Shard 1, 10001-20000 → Shard 2. Simple, supports range queries (BETWEEN). Problem: hot spots — new users get higher IDs, new shard gets most writes. Data skew if ranges poorly chosen. Good for time-series with time-based ranges."
    },
    {
      "heading": "Resharding (Rebalancing)",
      "text": "Adding shards requires moving data. Strategies: 1) Preshard: create more shards than needed initially (e.g., 1024 virtual shards mapped to fewer physical). 2) Double write: write to old + new shards during migration. 3) Offline migration: downtime. 4) Proxy-based: shard proxy handles routing, data migration in background."
    }
  ],
  ...
  "codeExamples": [
    {
      ...
      "description": "Hash-based shard routing with MD5 hash modulo shard count."
    },
    {
      "title": "Vitess Shard Configuration",
      "useCase": "Sharding with Vitess.",
      "code": "# Vitess keyspace with 4 shards\ncreate keyspace user_ks with\n  replication_factor: 3\n  shards: [-40,40-80,80-c0,c0-]  # 4 shards in hex range\n# Routing table\ncreate table users (\n  id bigint, name varchar(100), email varchar(200),\n  primary key (id)\n)\n# VSchema: defines sharding key\nalter vschema on user_ks.users add vindex hash(id) using hash;",
      "description": "Vitess shard configuration with hash-based vindex."
    },
    {
      "title": "Consistent Hashing Implementation",
      "useCase": "Minimal remapping on resharding.",
      ...
      "description": "Consistent hashing ring — minimizes data movement when adding/removing shards."
    },
    {
      "title": "Cross-Shard Query with scatter-gather",
      "useCase": "Query all shards, merge results.",
      "code": "async function searchUsersByName(name) {\n  const results = [];\n  // Query ALL shards (scatter-gather)\n  const promises = SHARDS.map(async (shard, i) => {\n    const pool = new Pool(shard);\n    const res = await pool.query(\n      \"SELECT * FROM users WHERE name ILIKE $1 LIMIT 100\",\n      [\"%\" + name + \"%\"]\n    );\n    return res.rows;\n  });\n  const shardResults = await Promise.all(promises);\n  // Merge and sort results\n  for (const rows of shardResults) results.push(...rows);\n  return results.sort((a, b) => b.id - a.id).slice(0, 100);\n}",
      "description": "Scatter-gather pattern for cross-shard queries — query all, merge results."
    }
  ],
  "mcqQuestions": [
    {
      "question": "Shard key determines?",
      "options": [
        "Data format",
      ...
      "answer": 1,
      "explanation": "Shard key → which shard a row belongs to."
    },
    {
      "question": "Hash sharding gives?",
      "options": [
        "Range query support",
        "Even distribution",
        "Hot spots",
      ...
      "answer": 1,
      "explanation": "Hash distributes data evenly."
    },
    {
      "question": "Consistent hashing minimizes?",
      "options": [
        "Query latency",
        "Data movement on resharding",
        "Storage",
        "CPU usage"
      ],
      "answer": 1,
      "explanation": "Minimizes data movement when shards change."
    },
    {
      "question": "What is a hot spot?",
      "options": [
      ...
      "answer": 1,
      "explanation": "Shard receiving disproportionate traffic."
    },
    {
      "question": "Cross-shard queries are?",
      "options": [
        "Easy",
      ...
      "answer": 1,
      "explanation": "Cross-shard requires querying all shards."
    },
    {
      "question": "Presharding creates?",
      "options": [
        "More shards than nodes",
        "Fewer shards than nodes",
        "Equal shards and nodes",
        "Dynamic shards"
      ],
      "answer": 0,
      "explanation": "Create more virtual shards than physical nodes for flexibility."
    },
    {
      "question": "Database Sharding — What reduces errors most?",
      "options": [
        "Automation",
        "Manual processes",
        "Rushing",
    ...
    {
      "question": "Database Sharding — What improves speed?",
      "options": [
    ...
    {
      "question": "Database Sharding — What is key for monitoring?",
      "options": [
    ...
    {
      "question": "Database Sharding — What ensures quality?",
      "options": [
