...
}`,description:"Consistent hashing with virtual nodes ensures only 1/N keys move when a node joins or leaves."},{title:"Redis Rate Limiter (Sliding Window)",useCase:"Atomic Lua script for distributed rate limiting",code:`const RATE_LIMIT_SCRIPT = \`
local key = KEYS[1]
...
}`,description:"Sliding window rate limiter using Redis sorted set. Atomic Lua script ensures race-condition-free counting."},{title:"URL Shortener Key Generation",useCase:"Base62 encoding with KGS",code:`const BASE62 = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
...
// Performance: nanoseconds per encoding, batch of 10K keys uses ~100KB memory`,description:"Base62 encoding generates short unique keys from numeric IDs. KGS pre-generates batches to avoid DB contention. 7 characters suffice for trillions of URLs."}],mcqQuestions:[{question:"What is the first step in a system design interview?",options:["Draw the database schema","Estimate scale (DAU, QPS, storage)","Choose the tech stack","Discuss trade-offs"],answer:1,explanation:"Always start with requirements and scale estimates to guide all subsequent decisions."},{question:"Which caching pattern checks the cache first, falls back to DB, then populates the cache?",options:["Write-Through","Cache-Aside","Write-Behind","Read-Through"],answer:1,explanation:"Cache-Aside: check cache → miss → read DB → populate cache. Most common pattern."},{question:"For a chat system with 1:1 messaging, which storage is best?",options:["MySQL","MongoDB","Cassandra (write-optimized, time-ordered)","PostgreSQL"],answer:2,explanation:"Cassandra excels at write-heavy, time-ordered data with horizontal scaling."},{question:"What does consistent hashing solve?",options:["Faster queries","Minimal key redistribution when nodes change","Data compression","Authentication"],answer:1,explanation:"Consistent hashing ensures only 1/N of keys move when a node is added or removed."},{question:"What is the purpose of a CDN?",options:["Store database backups","Cache static content at edge servers close to users","Run application code","Manage user authentication"],answer:1,explanation:"CDNs cache static assets (images, CSS, JS, video) at geographically distributed edge servers."},{question:"In the CAP theorem, what does the P stand for?",options:["Performance","Partition Tolerance","Persistence","Parallelism"],answer:1,explanation:"Partition Tolerance means the system continues to function despite network partitions between nodes."}]};export{e as practice_system_design};
